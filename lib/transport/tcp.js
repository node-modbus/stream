...
Transport.prototype.unwrap = function (data) {
	...
	var fc  = unwrapped.pdu.readUInt8(0);
	var key = [ unwrapped.transactionId, unwrapped.unitId, fc ].join(":");

	if (typeof this.callbacks[key] == "function") {
		unwrapped.callback = this.callbacks[key];
		delete this.callbacks[key];
	} else if (fc & 0x80 == 0x80) {
		// it's an exception, remove exception bit
		key = [ unwrapped.transactionId, unwrapped.unitId, fc & 0x0F ].join(":");
