...
BaseTransport.prototype.listen = function () {
	...
	var handle    = function (data) {
		...
		if (isHeartbeatPackage(data)) {
			transport.emit('elpiast-heartbeat');
			return
		}

    if (isHelloPackage(data)) {
      const deviceId = getDeviceIdFromHello(data);
      transport.emit('elpiast-hello', deviceId);
      return
    }
...
function isHelloPackage(data) {
	return data.indexOf('test') === 0;
}

function isHeartbeatPackage(data) {
	return data.slice(0, 1).equals(Buffer.from('FE', 'hex'));
}

function getDeviceIdFromHello(data) {
	return data.slice(4, 13).toString('ascii');
}
