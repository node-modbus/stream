...
BaseTransport.prototype.retrySend = function (data, retries, retry, next) {
	call_queue(this.beforewrite, function () {
		this.write(data, function () {
			call_queue(this.afterwrite, function () {
				if (retries > 0) {
					this.retryTimer = setTimeout(function () {
						this.retrySend(data, retries - 1, retry, next);
					}, retry);
				} else {
					this.retryTimer = setTimeout(function () {
		...
		if (typeof req.callback == "function") {
			...
			try {
				...
				if (typeof req.request.code == "number") {
					...
					this.emit("request", req.request.code, req, function (err) {
					...
					this.emit("request", event_name, req, function (err) {
						var args = Array.prototype.slice.apply(arguments, 1);
						...
						this.write(this.wrap(pdu[req.request.code].Response.build.apply(pdu[req.request.code].Response, data), req));
					});
				}
			} catch (err) {
				this.emit("error", err);
